...
const FileProcessPage = () => {
    ...
    useEffect(() => {
        ...
        console.log('inicat----',initialCategory)
        console.log(subcategories)
        // debugger
        if (category && category !== initialCategory && (subcategories.some(subcategory => subcategory !== '') || Files.length > 0)) {
        console.log('cat2res is ---',category)
        console.log('innit22 is ---',initialCategory)

        // console.log('fbwhfbsfjhnwjkfnsjewvewvvbjwevn')
            // debugger
            setShowModal(true)
        } else {
            setShowModal(false)
    ...
    const handbuttonclick = (resetState) => {
        // debugger
        console.log(typeof(resetState))
        console.log(category)
        setShowModal(false);
        if (resetState) {
            // debugger
            console.log('fwnlvkjndmlvnsdvnajvnv')
          setCategory('');
          setSubcategories(['']);
          setInitialCategory('');
        }else{
    ...
    const handleSubcategoryChange = (index, event) => {
        ...
        // console.log('fjfbwfbwnm',selectCategoryOption)
    ...
    const handleDeleteClick = (index) => {
        ...
        const filesInSubcat = Files.filter((file) => file && file.subcategory === subcatToDelete);
    ...
    // const categories = ['Travels', 'Category2', 'Category3', 'Category4', 'Category5','Category6','Category7'];
    // const subcategories1 = ['Culture& Heritage', 'Modern Tourist Attractions', 'Museums','Malls','Hawker & Food Markets','LuxuryShopping','Experiential Dining']
        ...
        <div className={Loading ? "blur-background" : ""}>
            ...
            <div className="category1">
                                ...
                                {selectCategoryOption
                                    ...
                                    .map(option => {
                                        return Object.keys(option)
                                            ...
                                            .filter(key => !subcategories.slice(0, index).includes(option[key])) 
                                            .map((key, subIndex) => (
                                                <option className="select-option" key={subIndex} value={option[key]}>
                                                    {option[key]}
                                                </option>
                                            ));
                                    })
                ...
                {/* {subcategories.map((subcategory, index) => (
                    ...
                    <div className="cat-lab" key={index}>
                        ...
                        >
                            ...
                            {selectCategoryOption
                                .filter(option => option.category === category) // Filter options based on selected category
                                .map(option => {
                                    return Object.keys(option)
                        ...
                        {/* <select className="select-size sub-category-container  sub-cat" value={subcategory} onChange={(e) => handleSubcategoryChange(index, e)} disabled={isCategorySelected}>
                        ...
                        {subcategories.length > 1 && (<DeleteButton onClick={(file) => handleDeleteClick(index)}>Delete</DeleteButton>)}
                ...
                <div className="submit-container">
                    ...
                    {/* <ModalAlert show={showResetCategoryModal} onClose={handleResetCategoryModal} onConfirm={resetCategory} /> */}
                    ...
                    {showAddSubAlert && <InfoAlert message="Please select File before adding another subcategory."/>}
                    ...
                    {showFileErrorAlert && <ErrorAlert message='Something is wrong with uploaded files. Please check your document content.'/>}
                    {showModal &&
                            <ModalAlert onSubmit={() => handbuttonclick(true)} onCancel={() => handbuttonclick(false)} onClose={() => handbuttonclick(false)}>Are you sure you want to change the category? If yes, your existing data will be lost.</ModalAlert>
                                }
                    {showDeleteModal &&
                            <ModalAlert onSubmit={() => handleDeleteConfirm(true)} onCancel={() => handleDeleteConfirm(false)} onClose={() => handleDeleteConfirm(false)}>Are you sure that you want to delete this document?</ModalAlert>
                                }
